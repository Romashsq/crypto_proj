// LessonPage.jsx - FIXED: enrollment only through CourseCard button
...
const LessonPage = () => {
  ...
  const courseIdAliases = { 'crypto-fundamentals': 'crypto' };
  const normalizedCourseId = courseIdAliases[courseId] || courseId;

  const lessonNum = parseInt(lessonId) || 1;
  const nextLessonNum = lessonNum + 1;
  const prevLessonNum = lessonNum - 1;
  const totalLessons = getTotalLessons(normalizedCourseId);
  const lessonData = getLessonData(normalizedCourseId, lessonNum);
  const nextLessonExists = hasNextLesson(normalizedCourseId, lessonNum);
  const nextTitle = getNextLessonTitle(normalizedCourseId, lessonNum);
  const prevLessonData = getLessonData(normalizedCourseId, prevLessonNum);
  ...
  useEffect(() => {
    const init = async () => {
      // Только проверяем статус — НЕ записываем автоматически
...
// Только проверяем — не записываем
  const checkEnrollmentStatus = async () => {
    try {
      if (!api.isAuthenticated()) return;
      const response = await api.checkEnrollment(normalizedCourseId);
      if (response.success) {
  ...
  // Записываемся только если пользователь явно нажал кнопку
  ...
  const checkLessonStatus = async () => {
    try {
      ...
      if (lessonNum === 1) {
        setCanAccess(true);
        try {
          const response = await api.getLessonStatus(normalizedCourseId, lessonNum);
          if (response.success) {
  ...
  const loadCourseProgress = async () => {
    try {
      if (!api.isAuthenticated()) return;
      const response = await api.getCourseProgress(normalizedCourseId);
  ...
  const handleMarkComplete = async () => {
    if (isCompleted || !lessonData || !canAccess || isMarking) return;

    setIsMarking(true);

    try {
      // Если не записан — записываем один раз перед завершением урока
      if (!isEnrolled) {
      ...
      const response = await api.completeLesson(normalizedCourseId, lessonNum, 300, 100);
        ...
        <div className={styles.lessonLayout}>
          ...
          <main className={styles.lessonContent}>
            <div className={styles.lockedLesson}>
              <div className={styles.lockedIcon}><i className="fas fa-lock"></i></div>
              <h2>Lesson Locked</h2>
              <p>Complete the previous lesson first.</p>
              <div className={styles.lockedActions}>
                <Link to={`/lesson/${courseId}/${lessonNum - 1}`} className={styles.goBackButton}>
                  <i className="fas fa-arrow-left"></i> Go to previous lesson
                </Link>
                <button className={styles.refreshButton} onClick={checkLessonStatus}>
  ...
  return (
    <div className={`${styles.lessonPage} ${theme === 'dark' ? styles.darkMode : ''}`}>
      <Toast message="Lesson marked as completed!" isVisible={showToast} duration={5000} />
          ...
          <div className={styles.lessonHeader}>
              ...
              <div className={styles.headerActions}>
                <div className={styles.enrollmentStatus}>
                  {isEnrolled ? (
                    ...
                    <button className={styles.enrollNowBtn} onClick={enrollToCourse}>
                      <i className="fas fa-plus"></i> Enroll in Course
                    </button>
                  )}
                </div>
                <SaveButton lesson={lessonForSave} size="medium" showLabel={true} variant="gradient" />
            ...
            <div className={styles.infoCardsGrid}>
              <div className={styles.infoCard}>
                <div className={styles.cardIcon}><i className="far fa-clock"></i></div>
              ...
              <div className={styles.infoCard}>
                <div className={styles.cardIcon}><i className="fas fa-signal"></i></div>
                <div className={styles.cardContent}>
              ...
              <div className={styles.infoCard}>
                <div className={styles.cardIcon}><i className="fas fa-hashtag"></i></div>
                <div className={styles.cardContent}>
              ...
              <div className={styles.infoCard}>
                <div className={styles.cardIcon}><i className="fas fa-bookmark"></i></div>
                <div className={styles.cardContent}>
            ...
            <div className={styles.videoCard}>
              <div className={styles.videoHeader}>
                ...
                <div className={styles.videoActions}>
                  <SaveButton lesson={lessonForSave} size="small" showLabel={false} />
            ...
            <div className={styles.completeCard}>
              <div className={styles.completeHeader}>
                <h4 className={styles.completeTitle}>
                  <i className="fas fa-flag-checkered"></i> Mark as Completed
                </h4>
                <SaveButton lesson={lessonForSave} size="small" showLabel={true} className={styles.sideSaveButton} />
              </div>

              <div className={styles.completeButtonWrapper}>
                <button
                  className={`${styles.completeButton} ${isCompleted ? styles.completed : ''} ${isMarking ? styles.marking : ''}`}
                ...
                >
                  <span className={styles.buttonContent}>
                    ...
                    <span className={styles.buttonText}>
                      {isCompleted ? 'Lesson Completed' : isMarking ? 'Marking...' : 'Mark as Completed'}
                    </span>
                  </span>
                  {!isCompleted && !isMarking && canAccess && (
                    <span className={styles.buttonArrow}><i className="fas fa-arrow-right"></i></span>
                  )}
                </button>
                ...
                {!isEnrolled && !isCompleted && (
                  <div className={styles.enrollmentMessage}>
                    ...
                    <button className={styles.enrollInlineBtn} onClick={enrollToCourse}>Enroll Now</button>
                  </div>
                )}

                {isCompleted && (
                  <div className={styles.completedMessage}>
                    <div className={styles.messageIcon}><i className="fas fa-check"></i></div>
            ...
            <div className={styles.navigationContainer}>
              <div className={styles.navigationGrid}>
                <Link
                  ...
                  className={`${styles.navButton} ${styles.prevButton} ${!prevLessonData ? styles.disabled : ''}`}
                >
                  <div className={styles.navButtonInner}>
                    <div className={styles.navIcon}><i className="fas fa-chevron-left"></i></div>
                    <div className={styles.navContent}>
                      <span className={styles.navLabel}>Previous Lesson</span>
                      <span className={styles.navTitle}>
                        {prevLessonData ? `Lesson ${prevLessonNum}: ${prevLessonData?.title || ''}` : 'Course Start'}
                      </span>
                    </div>
                  </div>
                </Link>
                ...
                <Link to={`/${courseId}`} className={`${styles.navButton} ${styles.overviewButton}`}>
                  <div className={styles.navButtonInner}>
                    <div className={styles.navIcon}><i className="fas fa-list-ul"></i></div>
                    <div className={styles.navContent}>
                ...
                <Link
                  ...
                  className={`${styles.navButton} ${styles.nextButton} ${(!nextLessonExists || !isCompleted) ? styles.disabled : ''}`}
                >
                  <div className={styles.navButtonInner}>
                    <div className={styles.navContent}>
                      <span className={styles.navLabel}>Next Lesson</span>
                      <span className={styles.navTitle}>
                        {nextLessonExists ? `Lesson ${nextLessonNum}: ${nextTitle || ''}` : 'Course Completed'}
                      </span>
                    </div>
                    <div className={styles.navIcon}><i className="fas fa-chevron-right"></i></div>
                  </div>
                </Link>
              </div>
              ...
              {nextLessonExists && !isCompleted && (
                <div className={styles.nextLessonHint}>
                  <div className={styles.hintIcon}><i className="fas fa-lock"></i></div>
                  <div className={styles.hintContent}>
                    <span className={styles.hintText}>Complete this lesson to unlock Lesson {nextLessonNum}</span>
                  </div>
                </div>
              )}
            </div>
